...
- `authority` — the light, light group, or (when a dumb load is hard-wired to a switch) the attached relay.
...
**Guarded writes.** Every write checks the target differs and is not `unavailable`/`unknown` first, to avoid command storms and echo loops.
